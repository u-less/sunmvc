...
$(function () {
    ...
    height = height - 46;
    $("#List").datagrid({
         url: '/admin/sysdicttype/GetDictPageJson/',
        title: '字典数据类别列表',
        width: 'auto',
        height: height,
        iconCls: 'icon-table',
        ...
        columns: [[{ field: 'DictId', width: 50, align: 'center', title: '编号' },
            ...
            { field: 'Code', width: 150, editor: 'text', align: 'center', editor: 'text', title: '编码' },
    ...
    $('#AddorUpdate').dialog({
        ...
        buttons: [{
            text: '保存', id: 'addBtn', iconCls: 'icon-save', handler: function () {
                $("#edit").form("submit", {
                     url: "/admin/sysdicttype/EditOrUpdateDict/",
                    onSubmit: function () {
...
var config = {
    add: {
        text: '新增字典数据类别', id: 'addBtn', iconCls: 'icon-add', handler: function () {
            $('#edit').form('clear');
            dataEdit('icon-add', '新增字典数据类别');
        }
    },
    edit: {
        text: '编辑字典数据类别', id: 'editBtn', iconCls: 'icon-edit', disabled: true, handler: function () {
            $("#edit").form("load", selectData);
            dataEdit('icon-edit', '编辑字典数据类别');
        }
    },
    remove: {
        text: '删除数据', id: 'deleteBtn', iconCls: 'icon-remove', disabled: true, handler: function () {
            $.messager.confirm("删除提醒", "确定删除吗？删除后将不可恢复", function (r) {
                if (r) {
                    $.ajax({
                        async: false,
                         url: "/admin/sysdicttype/DeleteDict/",
                        type: "post",
                        dataType: "html",
