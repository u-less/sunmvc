            ...
            if (data == 'True') {
                $.messager.confirm("操作提醒", "操作成功", function (r) {
                    ...
                    if (id == '' || parseInt(id) == 0) {
                        ...
                        if (window.parent.window.$('#tabs').tabs('exists', '编辑特产')) {
                            window.parent.window.$('#tabs').tabs("close", '编辑特产');
                        }
                    }
                });
            }
...
$(function () {
    var altras, altrasVal, bourns, bournsVal;
    $('#Bourn').combogrid({
        panelWidth: 600,
        panelHeight: 400,
        ...
        onBeforeLoad: function (param) {
            bournsVal = $('#Bourn').combo("getValues");
            var data = $('#Bourn').combogrid('grid').datagrid('getSelections');
            bourns = cloneObj(data);
        },
        onLoadSuccess: function (data) {
            if (typeof bourns === 'object') {
                var grid = $('#Bourn').combogrid('grid');
                $.each(bourns, function (i, v) {
                    grid.datagrid('appendRow', v);
                });
                if(bournsVal!='')
                    $('#Bourn').combogrid("setValues", bournsVal);
            }
        },
        toolbar: '#bourntool'
    });
    createBournsToolbar();
    $('#Bourn').combogrid('setValues', BournsArr);

    $('#Altradions').combogrid({
        panelWidth: 640,
        panelHeight: 400,
        ...
        { field: 'AltradionId', title: '编号', width: 60 },
            ...
            altrasVal = $('#Altradions').combo("getValues");
            var data = $('#Altradions').combogrid('grid').datagrid('getSelections');
            altras = cloneObj(data);
        },
        onLoadSuccess: function (data) {
            if (typeof altras === 'object') {
                var grid = $('#Altradions').combogrid('grid');
                $.each(altras, function (i, v) {
                    grid.datagrid('appendRow', v);
                });
                if (altrasVal != '')
                    $('#Altradions').combogrid("setValues", altrasVal);
            }
        },
        toolbar: '#altradionstool'
    });
    createAltradionsToolbar();
    $('#Altradions').combogrid('setValues', AltradionsArr);
...
function createBournsToolbar() {
    ...
    $("#bourntool>#btnSelect").linkbutton().click(function () {
        ...
        $("#Bourn").combogrid('grid').datagrid("load", { 'btype': BournType, 'btheme': BournTheme, 'bseason': BournSeason, 'bname': BournName });
    });
}
...
function createAltradionsToolbar() {
    ...
    $("#altradionstool>#btnSelect2").linkbutton().click(function () {
        ...
        $("#Altradions").combogrid('grid').datagrid("load", { 'theme': theme, 'season': season, 'name': name, 'level': level });
    });
}

function mulCancel(eid) {
    $('#' + eid).combogrid("clear");
}